...
class WxPayApi
{
    ...
    public static function unifiedOrder($inputObj, $timeOut = 6)
    {
        ...
        $val = $inputObj->GetValues();
        if (!isset($val['spbill_create_ip']))
    ...
    public static function Transfers($inputObj, $timeOut = 6)
    {
        ...
        if (!$inputObj->IsPartner_trade_no()) {
            throw new WxPayException("缺少商户订单号partner_trade_no！");
        } else if (!$inputObj->IsOpenidSet()) {
            throw new WxPayException("缺少用户openid！");
        } else if (!$inputObj->IsAmount()) {
            throw new WxPayException("缺少金额amount！");
        } else if (!$inputObj->IsDesc()) {
            throw new WxPayException("缺少企业付款备注desc！");
        }
        if (!$inputObj->IsCheck_name())
            $inputObj->SetCheck_name('NO_CHECK');//终端ip
        elseif ($inputObj->IsRe_user_name())
            throw new WxPayException("缺少收款人姓名！");
        ...
        $res = $result->FromXml($response);
        //   $result = WxPayResults: WxPayResults::Init($response);
        self::reportCostTime($url, $startTimeStamp, $result);//上报请求花费时间
        return $res;
    }
     ...
     * @throws WxPayException
     */
    public static function TransfersGet($inputObj, $timeOut = 6)
    {
    ...
    public static function SendRedPack($inputObj, $timeOut = 6)
    {
        ...
        if (!$inputObj->IsMch_billnoSet()) {
            throw new WxPayException("缺少商户订单号mch_billno！");
        } else if (!$inputObj->IsRe_openidSet()) {
            throw new WxPayException("缺少用户openid！");
        } else if (!$inputObj->IsTotal_amountSet()) {
            throw new WxPayException("缺少金额total_amount！");
        } else if (!$inputObj->IsRemarkSet()) {
            throw new WxPayException("缺少红包备注remark！");
        }
